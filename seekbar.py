...
class SeekBar(tk.Frame):
    ...
    def __init__(self, parent, mixer_controller: MixerController):
        ...
        self.canvas = tk.Canvas(self, width=400, height=30, bg=PLAY_BAR_COL, highlightthickness=0)
        ...
        self.line = self.canvas.create_line((10, 15, self.progress_bar_width, 15), width=5, fill=BATTLESHIP_GREY)
        # highlighted line which shows the current progress
        self.progress_line = self.canvas.create_line(10, 15, 10, 15, width=5, fill=BUTTON_COL)
        ...
        self.time_label = tk.Label(self, text="0:00 / 0:00", fg="white", bg=colour_scheme["dark"])
    ...
    def _update_progress_bar(self):
        ...
        self.canvas.coords(self.handle, xpos-10 + self.displacement, 5, xpos+10 + self.displacement, 25)
        self.canvas.coords(self.progress_line, 10, 15, xpos + self.displacement, 15)
    ...
    def _update_time_label(self, completed):
        ...
        if self.displacement:
            time_displacement = ((self.displacement + 10) / self.progress_bar_width) * duration_raw
        else:
            time_displacement = 0
        ...
        self.time_label.config(text=f"{formatted_current_time} / {formatted_duration}")

    def _on_click(self, event):  # User clicks on a position in the song to go to
    ...
    def _on_drag(self, event):  # User drags the progress bar's handle to change position in song
        self._on_click(event)

    def received_new_track_signal(self, track_id):
        """
        Resets attributes related to the progress bar when it receives a signal that the track changes.
        """
        self.displacement = 0
