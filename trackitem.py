...
class TrackItem(tk.Frame):
    ...
    """
    def __init__(self, parent, play_command, track_name, artist_name, artist_id, album, album_id, release_date, track_number, duration, add_to_queue_command, start_column=0, ):
    ...
    def _create_widgets(self):
        ...
        duration_label.grid(row=0, column=self.start_column+4, rowspan=2, padx=10)

        play_next_button = tk.Button(self, text="⏭️", bg=TRACK_LIST_COL, fg="white", relief="flat", font=("Arial", 16))
        play_next_button.grid(row=0, column=self.start_column+5, rowspan=2)

        add_to_queue_button = tk.Button(self, text="📋", bg=TRACK_LIST_COL, fg="white", relief="flat", font=("Arial", 16), command=self.add_to_queue_command)
        add_to_queue_button.grid(row=0, column=self.start_column+6, rowspan=2)

        add_to_playlist_button = tk.Button(self, text="➕", bg=TRACK_LIST_COL, fg="white", relief="flat", font=("Arial", 16))
        add_to_playlist_button.grid(row=0, column=self.start_column+7, rowspan=2)
