...
class QueueItem(TrackItem):
    def __init__(self, parent, play_command, track_name, artist_name, artist_id, album, album_id, release_date, track_number, duration, queue_index, reorder_queue_function, track_id, queue_max, add_to_queue_command):
        super().__init__(parent, play_command, track_name, artist_name, artist_id, album, album_id, release_date, track_number, duration, add_to_queue_command, start_column=1)
        ...
        move_up_button = tk.Button(self, text="⬆️", command=self.move_up, width=4, bg=TRACK_LIST_COL, fg=BUTTON_COL)
        move_up_button.grid(row=0, column=0)
        move_down_button = tk.Button(self, text="⬇️", command=self.move_down, width=4, bg=TRACK_LIST_COL, fg=BUTTON_COL)
